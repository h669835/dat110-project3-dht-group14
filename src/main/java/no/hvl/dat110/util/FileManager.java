...
public class FileManager {
	...
	public void createReplicaFiles() {
		...
		BigInteger[] svar = new BigInteger[numReplicas];
	...
	public Set<Message> requestActiveNodesForFile(String filename) throws RemoteException {
		...
		for (int i = 0; i < replicafiles.length; i++) {
			...
			NodeInterface successor = chordnode.findSuccessor(replicafiles[i]);
			activeNodesforFile.add(successor.getFilesMetadata(replicafiles[i]));
	...
	public NodeInterface findPrimaryOfItem() {
		...
		for (Message activeNode : activeNodesforFile) {
			// use the primaryServer boolean variable contained in the Message class to
			// check if it is the primary or not
			if (activeNode.isPrimaryServer()) {
				return Util.getProcessStub(activeNode.getNodeName(), activeNode.getPort());
			}
		}
