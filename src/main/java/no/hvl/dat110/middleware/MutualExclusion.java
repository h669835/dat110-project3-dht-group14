...
public class MutualExclusion {
	...
	public boolean doMutexRequest(Message message, byte[] updates) throws RemoteException {
		...
		boolean allMessagesReturned = areAllMessagesReturned(peers.size());
		// if yes, acquireLock
		if (allMessagesReturned) {
			acquireLock();
	...
	private void multicastMessage(Message message, List<Message> activenodes) throws RemoteException {
		...
		activenodes.forEach(peer -> {
			try {
				Util.getProcessStub(peer.getNodeName(), peer.getPort()).onMutexRequestReceived(message);
	...
	public void multicastReleaseLocks(Set<Message> activenodes) {
		...
		activenodes.forEach(peer -> {
			...
			try {
				Util.getProcessStub(peer.getNodeName(), peer.getPort()).releaseLocks();
			} catch (RemoteException e) {
	...
	private boolean areAllMessagesReturned(int numvoters) throws RemoteException {
		...
		int queueSize = queueack.size();

		// clear the queueack
		queueack.clear();

		// return true if yes and false if no
		return queueSize == numvoters;
